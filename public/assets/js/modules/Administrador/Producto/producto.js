...
$(function () {
  var tablaPrincipal = $("#tabla-producto").DataTable({
    ...
    columns: [
      ...
      {
        data: "ciudad",
        name: "cd.departamento",
        title: "Ciudad de Venta",
        orderable: true,
        searchable: true,
      },
      {
        data: "tipoProducto",
        name: "tp.tipo_producto",
...
var vm = new Vue({
  ...
  created: function () {
    axios.get(getTipoProducto).then((response) => {
      this.tipoProducto = response.data.data;
    });
    axios.get(getCiudadVenta).then((response) => {
  ...
  methods: {
    ...
    storeProducto() {
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (error.response.data.mostrar_mensaje)
            swal(error.response.data.message, { icon: "error" });
          if (!error.response.data.mostrar_formulario)
            $("#frmproducto").modal("hide");
        });
    },
    ...
    editProducto(id) {
      ...
      vm.editar = true;
      vm.errors = {};
      axios.get(datos_producto + "/" + id).then((response) => {
    ...
    deleteProducto(id) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(eliminar_producto + "/" + id).then((response) => {
            Swal.fire({
              position: "top-end",
              icon: "success",
          ...
          Swal.fire({
            ...
            icon: "error",
            title:
              "Hubo algun problema, contactese con el administrador de la UTIC",
            showConfirmButton: false,
            timer: 1500,
          });
          $("#frmverproducto").modal("show");
